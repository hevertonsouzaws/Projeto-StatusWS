<template>
    <div class="flex flex-col lg:flex-row items-center justify-between max-w-[100%] py-3 px-6">
        <div
            ...
            <div
                ...
                <img class="w-10 h-10 rounded-full" src="https://cdn-icons-gif.flaticon.com/17091/17091713.gif"
                    alt="Ícone Ativos">
            </div>
            <div v-for="status in statusCounts" :key="status.name" class="bg-gray-800 rounded-xl text-center py-2 px-4 border border-green-800 
                ...
                <div class="flex flex-col items-center justify-center gap-2 ml-2">
                    <img :src="status.iconUrl" class="w-10 h-10 rounded-full" :alt="status.name" />
                </div>
            </div>
        </div>
        <div class="text-right text-3xl w-[40%] px-8 mb-15">
            <h1 class="text-4xl text-white"><span class="text-red-600">#</span>SomosWS</h1>
            <p class="text-red-600 uppercase text-2xl">Unidos pela Inovação,</p>
            <p class="uppercase text-2xl">Impulsionando</p>
            <p class="text-red-600 uppercase text-2xl">O futuro!</p>
        </div>
    </div>
</template>
