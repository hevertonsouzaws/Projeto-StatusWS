<template>
  <div class="px-8 bg-gray-900 min-h-screen text-gray-100 flex flex-col justify-center items-center">
    <div class="glass-effect rounded-2xl p-6 text-center min-w-4xl max-w-7xl">
      ...
      <div v-else class="flex flex-wrap justify-center gap-8">
        <div v-for="employee in employees" :key="employee.employeeId" @click="handleLogin(employee)"
          class="flex flex-col items-center cursor-pointer transition-transform transform hover:scale-105 duration-300">
          <div
          ...
          <p class="mt-4 text-base font-medium text-white truncate w-28">{{ employee.name }}</p>
        </div>
      </div>
    </div>
  </div>
